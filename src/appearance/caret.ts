import { caret, letterContainer } from "../ui/dom"
...
export const setCaretMovement = (args: string[]): boolean => {
...
export const getCaretMovement = (): string => {
    return `${settings.slideOnWord ? 'word' : 'letter'} ${settings.slideSpeed}`
}

export const setCaret = function (style: string): boolean {
    if (style === 'u') {
        style = 'underscore'
    }
    if (style === 'n') {
